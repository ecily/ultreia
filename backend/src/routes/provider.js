...
function providerError(res, error) {
  ...
  if (message.includes('required') || message.includes('invalid') || message.includes('unknown Need') || message.includes('availability')) return badRequest(res, { message });
...
export function createProviderRouter(config, databaseService, providerService, googlePlacesService, authMiddleware) {
  ...
  router.post('/location/autocomplete', async (req, res) => {
    try {
      ...
      const result = await googlePlacesService.autocomplete({ input, sessionToken: readString(req.body?.sessionToken, { name: 'sessionToken', max: 200 }), locale: normalizeLocale(req.body?.locale) });
  ...
  router.post('/location/validate', async (req, res) => {
    try { const db = databaseRequired(res, databaseService); if (!db) return; return res.json({ ok: true, location: await providerService.validateLocation(req.body || {}, normalizeLocale(req.body?.sourceLocale)) }); } catch (error) { return providerError(res, error); }
  });
