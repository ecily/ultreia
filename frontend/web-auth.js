...
async function renderAdminLegacy(role) {
  if (role === 'provider') return renderProviderStart();
  let scope = webScope();
  const state = { user: null, overview: null, providers: [], offers: [], needs: [], quality: [], tab: 'overview', status: '', search: '' };
  const labels = { de: { overview: 'Übersicht', providers: 'Anbieter', offers: 'Angebote', needs: 'Needs', quality: 'Datenqualität', search: 'Suchen', all: 'Alle', loading: 'Wird geladen …', error: 'Daten konnten nicht geladen werden.', test: 'TESTDATEN – NICHT PRODUKTIV', noIssues: 'Keine berechenbaren Probleme gefunden.' }, en: { overview: 'Overview', providers: 'Providers', offers: 'Offers', needs: 'Needs', quality: 'Data quality', search: 'Search', all: 'All', loading: 'Loading …', error: 'Could not load data.', test: 'TEST DATA – NOT PRODUCTION', noIssues: 'No computable issues found.' }, es: { overview: 'Resumen', providers: 'Proveedores', offers: 'Ofertas', needs: 'Necesidades', quality: 'Calidad de datos', search: 'Buscar', all: 'Todos', loading: 'Cargando …', error: 'No se pudieron cargar los datos.', test: 'DATOS DE PRUEBA – NO PRODUCTIVO', noIssues: 'No se han encontrado problemas calculables.' } };
  Object.assign(labels.de, { block: 'Sperren', unblock: 'Entsperren', production: 'Produktion', localTestScope: 'local_test' }); Object.assign(labels.en, { block: 'Block', unblock: 'Unblock', production: 'Production', localTestScope: 'local_test' }); Object.assign(labels.es, { block: 'Bloquear', unblock: 'Desbloquear', production: 'Producción', localTestScope: 'local_test' });
  const at = () => labels[currentWebLanguage()] || labels.en;
  const load = async () => { const me = await webApi('/auth/me'); if (!me.user?.roles?.includes('admin') || me.session?.activeRole !== 'admin') throw new Error('role_access_not_granted'); state.user = me.user; const [overview, providers, offers, needs, quality] = await Promise.all([webApi(`/admin/overview?scope=${scope}`), webApi(`/admin/providers?scope=${scope}&status=${encodeURIComponent(state.status)}&search=${encodeURIComponent(state.search)}`), webApi(`/admin/offers?scope=${scope}&status=${encodeURIComponent(state.status)}`), webApi('/admin/needs'), webApi(`/admin/quality?scope=${scope}`)]); state.overview = overview; state.providers = providers.items || []; state.offers = offers.items || []; state.needs = needs.items || []; state.quality = quality.items || []; };
  const statusBadge = (value) => `<span class="provider-offer-status status-${escapeProviderHtml(value)}">${escapeProviderHtml(value)}</span>`;
  const render = () => { const l = at(); const counts = state.overview?.providers || {}; const offerCounts = state.overview?.offers || {}; const content = state.tab === 'overview' ? `<div class="admin-stat-grid"><article><strong>${counts.active || 0}</strong><span>${l.providers} · active</span></article><article><strong>${counts.pending || 0}</strong><span>${l.providers} · pending</span></article><article><strong>${offerCounts.active || 0}</strong><span>${l.offers} · active</span></article><article><strong>${state.quality.length}</strong><span>${l.quality}</span></article></div><section class="admin-table-panel"><h2>${l.overview}</h2><p>${scope === 'local_test' ? l.test : ''}</p><pre class="admin-summary">${escapeProviderHtml(JSON.stringify({ providers: counts, offers: offerCounts, needs: state.overview?.needs }, null, 2))}</pre></section>` : state.tab === 'providers' ? `<section class="admin-table-panel"><h2>${l.providers}</h2><div class="admin-filter"><input data-admin-search value="${escapeProviderHtml(state.search)}" placeholder="${l.search}"><select data-admin-status><option value="">${l.all}</option>${statusesForAdmin().map((status) => `<option value="${status}" ${state.status === status ? 'selected' : ''}>${status}</option>`).join('')}</select></div><div class="admin-list">${state.providers.map((item) => `<article class="admin-list-row"><div><strong>${escapeProviderHtml(item.businessName || item.email)}</strong><span>${escapeProviderHtml(item.email || '')} · ${escapeProviderHtml(item.scope)} · ${statusBadge(item.status)}</span><small>${escapeProviderHtml(item.location?.formattedAddress || 'No confirmed location')} · ${item.offerCount} offers</small></div><div><button type="button" data-admin-provider-status="${item.id}" data-status="${item.status === 'blocked' ? 'active' : 'blocked'}">${item.status === 'blocked' ? 'Unblock' : 'Block'}</button></div></article>`).join('') || `<p>${l.noIssues}</p>`}</div></section>` : state.tab === 'offers' ? `<section class="admin-table-panel"><h2>${l.offers}</h2><div class="admin-filter"><select data-admin-status><option value="">${l.all}</option>${statusesForAdmin().map((status) => `<option value="${status}" ${state.status === status ? 'selected' : ''}>${status}</option>`).join('')}</select></div><div class="admin-list">${state.offers.map((item) => `<article class="admin-list-row"><div><strong>${escapeProviderHtml(item.title)}</strong><span>${escapeProviderHtml(item.providerName || '')} · ${escapeProviderHtml(item.scope)} · ${statusBadge(item.status)}</span><small>${escapeProviderHtml((item.needKeys || []).join(', '))} · ${escapeProviderHtml(item.radiusMeters)} m</small></div><div><button type="button" data-admin-offer-status="${item.id}" data-status="${item.status === 'blocked' ? 'active' : 'blocked'}">${item.status === 'blocked' ? 'Unblock' : 'Block'}</button></div></article>`).join('')}</div></section>` : state.tab === 'needs' ? `<section class="admin-table-panel"><h2>${l.needs}</h2><div class="admin-list">${state.needs.map((item) => `<article class="admin-list-row"><div><strong>${escapeProviderHtml(item.key)}</strong><span>${escapeProviderHtml(item.labels?.de || item.label || '')}</span><small>${escapeProviderHtml(item.criticality || '')} · pushable=${String(item.pushable !== false)} · ${item.status || 'active'}</small></div></article>`).join('')}</div></section>` : `<section class="admin-table-panel"><h2>${l.quality}</h2><div class="admin-list">${state.quality.map((item) => `<article class="admin-list-row"><div><strong>${escapeProviderHtml(item.type)}</strong><span>${escapeProviderHtml(item.label || item.entityId)}</span><small>${escapeProviderHtml(item.scope)}</small></div></article>`).join('') || `<p>${l.noIssues}</p>`}</div></section>`; webShell(tx('admin'), `<div class="admin-shell"><header class="admin-header"><div><p class="section-label">${tx('admin')}</p><h1>${escapeProviderHtml(state.user?.email || '')}</h1></div><span class="scope-badge">${escapeProviderHtml(scope)}</span><button class="web-auth-button secondary" data-admin-logout type="button">${tx('logout')}</button></header>${scope === 'local_test' ? `<div class="provider-test-banner">${l.test}</div>` : ''}<nav class="admin-nav">${['overview','providers','offers','needs','quality'].map((tab) => `<button type="button" data-admin-tab="${tab}" class="${state.tab === tab ? 'is-active' : ''}">${l[tab]}</button>`).join('')}</nav>${content}</div>`); bind(); };
  const statusesForAdmin = () => ['active', 'paused', 'draft', 'expired', 'blocked', 'pending'];
  const bind = () => { document.querySelectorAll('[data-admin-tab]').forEach((button) => button.addEventListener('click', () => { state.tab = button.dataset.adminTab; render(); })); document.querySelector('[data-admin-logout]')?.addEventListener('click', async () => { await webApi('/auth/logout', { method: 'POST' }, false).catch(() => {}); window.location.replace('/admin/login/'); }); document.querySelector('[data-admin-status]')?.addEventListener('change', async (event) => { state.status = event.target.value; await load(); render(); }); document.querySelector('[data-admin-search]')?.addEventListener('change', async (event) => { state.search = event.target.value; await load(); render(); }); document.querySelectorAll('[data-admin-provider-status]').forEach((button) => button.addEventListener('click', async () => { button.disabled = true; await webApi(`/admin/providers/${button.dataset.adminProviderStatus}/status`, { method: 'POST', body: JSON.stringify({ status: button.dataset.status, scope }) }); await load(); render(); })); document.querySelectorAll('[data-admin-offer-status]').forEach((button) => button.addEventListener('click', async () => { button.disabled = true; await webApi(`/admin/offers/${button.dataset.adminOfferStatus}/status`, { method: 'POST', body: JSON.stringify({ status: button.dataset.status, scope }) }); await load(); render(); })); };
  try { await load(); render(); } catch { window.location.replace('/admin/login/'); }
}
...
async function renderStart(role) {
  ...
  const copy = {
    de: { overview: 'Übersicht', providers: 'Provider', offers: 'Angebote', needs: 'Bedarfe', quality: 'Datenqualität', production: 'Produktion', local: 'local_test', search: 'Suchen', all: 'Alle', block: 'Sperren', unblock: 'Entsperren', save: 'Speichern', loading: 'Wird geladen …', error: 'Aktion konnte nicht verarbeitet werden.', test: 'TESTDATEN – NICHT PRODUKTIV', empty: 'Keine Einträge.' },
    en: { overview: 'Overview', providers: 'Providers', offers: 'Offers', needs: 'Needs', quality: 'Data quality', production: 'Production', local: 'local_test', search: 'Search', all: 'All', block: 'Block', unblock: 'Unblock', save: 'Save', loading: 'Loading …', error: 'The action could not be processed.', test: 'TEST DATA – NOT PRODUCTION', empty: 'No entries.' },
    es: { overview: 'Resumen', providers: 'Proveedores', offers: 'Ofertas', needs: 'Necesidades', quality: 'Calidad de datos', production: 'Producción', local: 'local_test', search: 'Buscar', all: 'Todos', block: 'Bloquear', unblock: 'Desbloquear', save: 'Guardar', loading: 'Cargando …', error: 'No se pudo procesar la acción.', test: 'DATOS DE PRUEBA – NO PRODUCTIVO', empty: 'Sin entradas.' }
  };
  const t = () => copy[language()] || copy.en;
  const scope = webScope();
  const state = { user: null, tab: 'overview', status: '', search: '', data: {}, feedback: '' };
  const load = async () => {
    const me = await webApi('/auth/me');
    if (!me.user?.roles?.includes('admin') || me.session?.activeRole !== 'admin') throw new Error('role_access_not_granted');
    state.user = me.user;
    const [overview, providers, offers, needs, quality] = await Promise.all([
      webApi(`/admin/overview?scope=${scope}`), webApi(`/admin/providers?scope=${scope}&status=${encodeURIComponent(state.status)}&search=${encodeURIComponent(state.search)}`),
      webApi(`/admin/offers?scope=${scope}&status=${encodeURIComponent(state.status)}`), webApi('/admin/needs'), webApi(`/admin/quality?scope=${scope}`)
    ]);
    state.data = { overview, providers: providers.items || [], offers: offers.items || [], needs: needs.items || [], quality: quality.items || [] };
  };
  const status = (value) => `<span class="provider-offer-status status-${escapeProviderHtml(value)}">${escapeProviderHtml(value)}</span>`;
  const render = () => {
    const l = t(); const d = state.data; const overview = d.overview || {}; let body = '';
    if (state.tab === 'overview') body = `<div class="admin-stat-grid"><article><strong>${overview.providers?.active || 0}</strong><span>${l.providers} · active</span></article><article><strong>${overview.providers?.pending || 0}</strong><span>${l.providers} · pending</span></article><article><strong>${overview.offers?.active || 0}</strong><span>${l.offers} · active</span></article><article><strong>${d.quality?.length || 0}</strong><span>${l.quality}</span></article></div>`;
    if (state.tab === 'providers') body = `<section class="admin-table-panel"><h2>${l.providers}</h2><div class="admin-filter"><input data-admin-search value="${escapeProviderHtml(state.search)}" placeholder="${l.search}"><select data-admin-status><option value="">${l.all}</option>${['active','pending','paused','blocked','expired','draft'].map((v) => `<option value="${v}" ${state.status === v ? 'selected' : ''}>${v}</option>`).join('')}</select></div><div class="admin-list">${d.providers.map((item) => `<article class="admin-list-row"><div><strong>${escapeProviderHtml(item.businessName || item.email || '')}</strong><span>${escapeProviderHtml(item.email || '')} · ${status(item.status)}</span><small>${escapeProviderHtml(item.location?.formattedAddress || '—')} · ${item.offerCount || 0} offers</small></div><button type="button" data-admin-provider-status="${item.id}" data-status="${item.status === 'blocked' ? 'active' : 'blocked'}">${item.status === 'blocked' ? l.unblock : l.block}</button></article>`).join('') || `<p>${l.empty}</p>`}</div></section>`;
    if (state.tab === 'offers') body = `<section class="admin-table-panel"><h2>${l.offers}</h2><div class="admin-filter"><select data-admin-status><option value="">${l.all}</option>${['active','pending','paused','blocked','expired','draft'].map((v) => `<option value="${v}" ${state.status === v ? 'selected' : ''}>${v}</option>`).join('')}</select></div><div class="admin-list">${d.offers.map((item) => `<article class="admin-list-row"><div><strong>${escapeProviderHtml(item.title || '')}</strong><span>${escapeProviderHtml(item.providerName || '')} · ${status(item.status)}</span><small>${escapeProviderHtml((item.needKeys || []).join(', '))} · ${escapeProviderHtml(item.radiusMeters)} m</small></div><button type="button" data-admin-offer-status="${item.id}" data-status="${item.status === 'blocked' ? 'active' : 'blocked'}">${item.status === 'blocked' ? l.unblock : l.block}</button></article>`).join('') || `<p>${l.empty}</p>`}</div></section>`;
    if (state.tab === 'needs') body = `<section class="admin-table-panel"><h2>${l.needs}</h2><div class="admin-list">${d.needs.map((item) => `<article class="admin-list-row"><div><strong>${escapeProviderHtml(item.key)}</strong><span>${escapeProviderHtml(item.labels?.de || item.label || '')}</span><small>${escapeProviderHtml(item.criticality || '')} · ${item.status || 'active'} · pushable=${String(item.pushable !== false)}</small></div></article>`).join('') || `<p>${l.empty}</p>`}</div></section>`;
    if (state.tab === 'quality') body = `<section class="admin-table-panel"><h2>${l.quality}</h2><div class="admin-list">${d.quality.map((item) => `<article class="admin-list-row"><div><strong>${escapeProviderHtml(item.type)}</strong><span>${escapeProviderHtml(item.label || item.entityId)}</span><small>${escapeProviderHtml(item.scope)}</small></div></article>`).join('') || `<p>${l.empty}</p>`}</div></section>`;
    webShell(tx('admin'), `<div class="admin-shell"><header class="admin-header"><div><p class="section-label">${tx('admin')}</p><h1>${escapeProviderHtml(state.user?.email || '')}</h1></div><div class="admin-scope-switch"><button type="button" data-admin-scope="production" class="${scope === 'production' ? 'is-active' : ''}">${l.production}</button><button type="button" data-admin-scope="local_test" class="${scope === 'local_test' ? 'is-active' : ''}">${l.local}</button></div><button class="web-auth-button secondary" data-admin-logout type="button">${tx('logout')}</button></header>${scope === 'local_test' ? `<div class="provider-test-banner">${l.test}</div>` : ''}<nav class="admin-nav">${['overview','providers','offers','needs','quality'].map((tab) => `<button type="button" data-admin-tab="${tab}" class="${state.tab === tab ? 'is-active' : ''}">${l[tab]}</button>`).join('')}</nav>${state.feedback ? `<p class="admin-feedback" role="status">${escapeProviderHtml(state.feedback)}</p>` : ''}${body}</div>`);
    bind();
  };
  const refresh = async () => { state.feedback = t().loading; render(); try { await load(); state.feedback = ''; render(); } catch { state.feedback = t().error; render(); } };
  const bind = () => {
    document.querySelectorAll('[data-admin-tab]').forEach((button) => button.addEventListener('click', () => { state.tab = button.dataset.adminTab; render(); }));
    document.querySelectorAll('[data-admin-scope]').forEach((button) => button.addEventListener('click', async () => { await webApi('/auth/session/switch-scope', { method: 'POST', body: JSON.stringify({ scope: button.dataset.adminScope }) }); window.sessionStorage.setItem(WEB_SCOPE_KEY, button.dataset.adminScope); window.location.reload(); }));
    document.querySelector('[data-admin-logout]')?.addEventListener('click', async () => { await webApi('/auth/logout', { method: 'POST' }, false).catch(() => {}); window.location.replace('/admin/login/'); });
    document.querySelector('[data-admin-status]')?.addEventListener('change', async (event) => { state.status = event.target.value; await refresh(); });
    document.querySelector('[data-admin-search]')?.addEventListener('change', async (event) => { state.search = event.target.value; await refresh(); });
    document.querySelectorAll('[data-admin-provider-status], [data-admin-offer-status]').forEach((button) => button.addEventListener('click', async () => { button.disabled = true; const isProvider = button.hasAttribute('data-admin-provider-status'); const id = button.dataset[isProvider ? 'adminProviderStatus' : 'adminOfferStatus']; const path = isProvider ? `/admin/providers/${id}/status` : `/admin/offers/${id}/status`; try { await webApi(path, { method: 'POST', body: JSON.stringify({ status: button.dataset.status, scope }) }); state.feedback = ''; await load(); render(); } catch { state.feedback = l.error; render(); } }));
  };
  try { await load(); render(); } catch { window.location.replace('/admin/login/'); }
}
