...
async function renderProviderStart() {
  const state = { account: null, profile: null, offers: [], needs: [], locationDraft: null, editingOffer: null };
  const load = async () => {
  ...
  const render = () => {
    ...
    webShell(tx('provider'), `${testBanner}${steps}<div class="provider-account"><strong>${escapeProviderHtml(profile.displayName || profile.contactEmail || '')}</strong><span>${escapeProviderHtml(profile.contactEmail || '')}</span><span class="scope-badge">${escapeProviderHtml(scope)}</span>${scopeSwitcher}<button class="web-auth-button secondary" data-logout type="button">${tx('logout')}</button></div>${profile.businessName ? profileForm : profileForm}${locationForm}${offerSection}`);
    bind();
  };
  const bind = () => {
    ...
    const autocomplete = window.UltreiaProviderAutocomplete?.create({
      ...
      onError: (error) => { document.querySelector('[data-location-message]').textContent = error.status === 'google_places_not_configured' ? pt('googleMissing') : pt('providerError'); },
    });
    locationInput?.addEventListener('input', () => autocomplete?.schedule(locationInput.value, currentWebLanguage()));
    document.querySelector('[data-suggestions]')?.addEventListener('click', async (event) => { const button = event.target.closest('[data-place-id]'); if (!button) return; autocomplete?.cancel(); try { const result = await webApi('/provider/location/validate', { method: 'POST', body: JSON.stringify({ googlePlaceId: button.dataset.placeId, sourceLocale: currentWebLanguage(), sessionToken: autocomplete?.sessionToken }) }); state.locationDraft = result.location; render(); } catch { document.querySelector('[data-location-message]').textContent = pt('providerError'); } });
    document.querySelector('[data-save-location]')?.addEventListener('click', async () => { if (!state.locationDraft && !state.profile?.location) return; const location = state.locationDraft || state.profile.location; const latitude = document.querySelector('[name="finalLatitude"]')?.value; const longitude = document.querySelector('[name="finalLongitude"]')?.value; const message = document.querySelector('[data-location-message]'); try { await webApi('/provider/location', { method: 'PUT', body: JSON.stringify({ googlePlaceId: location.googlePlaceId, finalLatitude: latitude ? Number(latitude) : undefined, finalLongitude: longitude ? Number(longitude) : undefined, sourceLocale: currentWebLanguage(), sessionToken: autocomplete?.sessionToken }) }); state.locationDraft = null; await load(); render(); } catch { message.textContent = pt('providerError'); } });
